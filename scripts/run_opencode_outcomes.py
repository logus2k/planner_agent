...
# Deterministic stub/mock fingerprints — a fake implementation, no LLM needed.
_STUB_PATTERNS = [
    r"\bTODO\b", r"\bFIXME\b", r"NotImplemented", r"raise\s+NotImplementedError",
    r"console\.log\(['\"][^'\"]*API", r"setTimeout\(", r"\bsimulat", r"\bmock\b",
    r"\bdummy\b", r"\bplaceholder\b", r"pass\s*#\s*implement", r"return\s+None\s*#",
]
_STUB_RE = re.compile("|".join(_STUB_PATTERNS), re.IGNORECASE)


def stub_signals(files, workdir):
    """Count deterministic stub fingerprints across the produced files."""
    hits = []
    for f in files:
        try:
            ...
            continue
        for m in _STUB_RE.finditer(body):
